...
if (isset($_POST['add_language'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM typing_languages WHERE language_code = :code");
        $stmt->execute([':code' => $language_code]);
        if ($stmt->fetchColumn() > 0) {
            $message = "Language code already exists!";
            $messageType = "danger";
        } else {
            $sql = "INSERT INTO typing_languages (language_name, language_code) VALUES (:name, :code)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([':name' => $language_name, ':code' => $language_code]);
...
<head>
    <meta charset="UTF-8">
    <title>Add New Language - Admin</title>
        ...
        <div id="page-content-wrapper" style="margin-left: 280px;">
            <div class="container-fluid py-5 px-lg-5">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="mb-0">Add New Language</h2>
                        ...
                        <form method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="language_name" class="form-control" placeholder="e.g. English" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">Language Code <span class="text-danger">*</span></label>
                                    <input type="text" name="language_code" class="form-control" placeholder="e.g. en" required>
                                </div>
                            </div>
                            <div class="d-flex justify-content-end">
                                <button type="submit" name="add_language" class="btn btn-primary btn-lg"><i class="fas fa-save me-2"></i> Save Language</button>
                            </div>
                        </form>
                    </div>
                </div>
